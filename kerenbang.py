...
# Konstanta
BASE_URL = "https://play.mobilelegends.com/v2.1/inapp/"
API_URL_LOGIN = f"{BASE_URL}login"
API_URL_CHANGE_EMAIL = f"{BASE_URL}changebindemail"
BOT_TOKEN = "7710828121:AAGdQmVhqQTFquxqwJ00BL_h_-vnWZ21ltw"  # Ganti dengan token bot Telegram Anda
...
def login(account, password, verification_code):
    ...
    login_data = {
        "op": "login",
        "sign": "ca62428dca478c20b860f65cf000201f",  # Sesuaikan jika diperlukan
        "params": {
            "account": account,
            "md5pwd": md5pwd,
            "game_token": "",
...
def change_email(game_token, guid, token, new_email, verification_code_new_email):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Change email failed: {response.text}")
    except requests.RequestException as e:
